...
const App = {
  ...
  setupUI() {
    ...
    if(clock) {
      setInterval(() => {
        const d = new Date();
        clock.innerHTML = `<div style="text-align:center; animation: fadeInUp 0.5s ease-out;">
  ...
  async startSpeedTest() {
    ...
    testStatus.textContent = 'Memulai test kecepatan...';
    
    try {
      // Test Ping
      testStatus.textContent = 'Mengukur latency...';
      const ping = await this.measurePing();
      pingValue.textContent = ping + ' ms';
      await this.delay(500);
      
      // Test Download Speed
      testStatus.textContent = 'Menguji kecepatan download...';
      const downloadMbps = await this.measureDownloadSpeed((speed) => {
      ...
      // Test Upload Speed (simulated since we can't actually upload)
      testStatus.textContent = 'Menguji kecepatan upload...';
      const uploadMbps = Math.max(downloadMbps * 0.1, downloadMbps * 0.05); // Typical upload is 10-50% of download
      uploadSpeed.textContent = uploadMbps.toFixed(2);
      
      testStatus.textContent = '✅ Test selesai!';
      this.showNotification('Speed test berhasil! 🎉', 'success');
  ...
  async measureDownloadSpeed(callback) {
    const testSizes = [1, 2, 5, 10, 20]; // MB
    let bestSpeed = 0;
    
    for (let size of testSizes) {
      try {
        const speed = await this.downloadTest(size, callback);
        bestSpeed = Math.max(bestSpeed, speed);
        
        // If we get a very high speed, we can stop early
        if (speed > 100) break;
      } catch (error) {
  ...
  async downloadTest(sizeMB, callback) {
    const startTime = performance.now();
    const sizeBytes = sizeMB * 1024 * 1024;
    
    try {
      // Use multiple test URLs for better accuracy
      const testUrls = [
        `https://httpbin.org/bytes/${sizeBytes}`,
        `https://picsum.photos/${Math.sqrt(sizeBytes * 8)}/${Math.sqrt(sizeBytes * 8)}`,
      ...
      for (let url of testUrls) {
        try {
          ...
          while (true) {
            ...
            if (callback) callback(speedMbps);
            
            // Update gauge
            const gaugeFill = document.getElementById('gaugeFill');
            this.updateGauge(gaugeFill, speedMbps);
          }
          
          const totalTime = (performance.now() - startTime) / 1000;
          const finalSpeed = (sizeBytes * 8) / (totalTime * 1000000);
          return finalSpeed;
      ...
      return this.simulateSpeedTest(callback);
  ...
  simulateSpeedTest(callback) {
    return new Promise((resolve) => {
      ...
      const interval = setInterval(() => {
        ...
        // Simulate increasing speed with some randomness
        speed = 10 + progress * 50 + Math.random() * 20;
        speed = Math.min(speed, 100); // Cap at 100 Mbps
